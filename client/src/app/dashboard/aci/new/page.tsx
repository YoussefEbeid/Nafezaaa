      ...
      <nav aria-label="Progress" className="overflow-x-auto pb-2">
        <ol role="list" className="flex items-center min-w-max">
          {steps.map((step, index) => {
            ...
            return (
              <li key={step} className={cn("relative flex-shrink-0", index !== steps.length - 1 && "pr-4 sm:pr-8 md:pr-20")}>
                <div className="flex items-center">
                  <button
                    onClick={() => canNavigate && setCurrentStep(index)}
                    disabled={!canNavigate}
                    className={cn(
                      "relative flex h-8 w-8 sm:h-10 sm:w-10 items-center justify-center rounded-full transition-colors",
                      isCompleted || isCurrent ? "bg-nafeza-600 hover:bg-nafeza-900" : "bg-gray-200",
                      canNavigate && "cursor-pointer",
                      !canNavigate && "cursor-not-allowed"
                    )}
                  >
                    {isCompleted ? <Check className="h-4 w-4 sm:h-5 sm:w-5 text-white" /> : <span className={cn("text-xs sm:text-sm font-bold", isCurrent ? "text-white" : "text-gray-500")}>{index + 1}</span>}
                  </button>
                  <button
                    onClick={() => canNavigate && setCurrentStep(index)}
                    disabled={!canNavigate}
                    className={cn(
                      "ml-2 sm:ml-4 text-xs sm:text-sm font-medium hidden sm:block",
                ...
                {index !== steps.length - 1 && (
                  <div className="absolute top-4 sm:top-5 left-8 sm:left-10 h-0.5 w-full -z-10 bg-gray-200 hidden sm:block">
          ...
          {currentStep === 2 && (
            <div className="space-y-6">
              ...
              <div className="flex flex-col sm:flex-row justify-between gap-3 sm:gap-2">
                ...
                <Button size="lg" onClick={handleFinalSubmit} isLoading={submitAci.isPending} className="w-full sm:w-auto">
