  ...
  return (
    <div className="space-y-4 sm:space-y-6 lg:space-y-8">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h1 className="text-2xl sm:text-3xl font-bold text-nafeza-700">{t('dashboard.aciRequests')}</h1>
          <p className="text-sm sm:text-base text-slate-500 mt-1">{language === 'ar' ? 'إدارة طلبات معلومات الشحن المسبق' : 'Manage your Advance Cargo Information requests'}</p>
        </div>
      ...
      <Card className="border-none shadow-md">
        <CardHeader>
          <CardTitle className="text-lg sm:text-xl">{t('dashboard.allRequests')}</CardTitle>
        </CardHeader>
        <CardContent>
          {isLoading ? (
            ...
            <>
              ...
              <div className="md:hidden space-y-4">
                {requests.map((request) => (
                  <div key={request.id} className="bg-white border border-slate-200 rounded-lg p-4 space-y-3">
                    <div className="flex items-start justify-between">
                      <div className="flex-1 min-w-0">
                        <p className="text-xs text-slate-500 mb-1">ACID Number</p>
                        <p className="font-mono text-sm font-medium text-nafeza-600 break-all">{request.acidNumber}</p>
                      </div>
                      <span className={`${getStatusColor(request.status)} text-xs font-medium px-2.5 py-0.5 rounded flex-shrink-0`}>
                        {request.status}
                      </span>
                    </div>
                    <div>
                      <p className="text-xs text-slate-500 mb-1">Exporter</p>
                      <p className="text-sm text-slate-800">{request.exporterName}</p>
                    </div>
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <p className="text-xs text-slate-500 mb-1">Date</p>
                        <p className="text-sm text-slate-800">{formatDate(request.requestDate)}</p>
                      </div>
                      <div>
                        <p className="text-xs text-slate-500 mb-1">Items</p>
                        <p className="text-sm text-slate-800">{request.itemCount}</p>
                      </div>
                    </div>
                    <Button 
                      variant="outline" 
