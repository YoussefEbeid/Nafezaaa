...
export default function TariffSearchPage() {
  ...
  return (
    <div className="space-y-4 sm:space-y-6 lg:space-y-8">
      {/* Header */}
      <div>
        <h1 className="text-2xl sm:text-3xl font-bold text-nafeza-700">{t('dashboard.tariffSearch')}</h1>
        <p className="text-sm sm:text-base text-slate-500 mt-1">{t('dashboard.lookupHSCodes')}</p>
      </div>
          ...
          <div className="space-y-4 sm:space-y-6">
            {activeTab === 'code' ? (
              <div className="space-y-4">
                ...
                <div className={`flex flex-col sm:flex-row gap-3 sm:gap-4 ${language === 'ar' ? 'sm:flex-row-reverse' : ''}`}>
                  <Input
            ...
            {results.length > 0 && (
              <>
                {/* Desktop Table View */}
                <div className="hidden md:block mt-8 border rounded-lg overflow-hidden">
                    ...
                    <div key={i} className="bg-white border border-slate-200 rounded-lg p-4 space-y-3">
                      <div className="flex items-start justify-between">
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="font-mono text-sm font-bold text-nafeza-600">{item.hsCode}</p>
                        </div>
                        <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium flex-shrink-0 ${item.dutyRate === 0 ? 'bg-green-100 text-green-800' : 'bg-blue-100 text-blue-800'}`}>
                          {item.dutyRate}%
                        </span>
                      </div>
                      <div>
