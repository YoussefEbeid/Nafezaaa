  ...
  return (
    <div className="space-y-4 sm:space-y-6 lg:space-y-8 animate-in fade-in duration-500">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h1 className="text-2xl sm:text-3xl font-bold text-nafeza-700">{t('dashboard.overview')}</h1>
          <p className="text-sm sm:text-base text-slate-500 mt-1">{language === 'ar' ? `مرحباً، ${user?.name || 'تاجر'}. إليك ما يحدث اليوم.` : `Hello, ${user?.name || 'Trader'}. Here is what's happening today.`}</p>
        </div>
        ...
        {stats.map((stat, index) => (
          <Card key={index} className="hover:shadow-lg transition-all duration-300 border-none shadow-md">
            <CardContent className="p-4 sm:p-6">
              ...
              <div className={`mt-3 sm:mt-4 flex items-center text-xs text-green-600 font-medium ${language === 'ar' ? 'flex-row-reverse' : ''}`}>
                <TrendingUp className={`h-3 w-3 ${language === 'ar' ? 'ml-1' : 'mr-1'}`} /> <span className="hidden sm:inline">+2.5% from last month</span><span className="sm:hidden">+2.5%</span>
              </div>
            </CardContent>
          </Card>
        ))}
      ...
      <Card className="border-none shadow-md">
        <CardHeader>
          <CardTitle className="text-lg sm:text-xl">{t('dashboard.recentDeclarations')}</CardTitle>
        </CardHeader>
        <CardContent>
          {isLoading ? (
            ...
            <>
              ...
              <div className="md:hidden space-y-4">
                {recentDeclarations.map((declaration) => (
                  <div key={declaration.id} className="bg-white border border-slate-200 rounded-lg p-4 space-y-3">
                    <div className="flex items-start justify-between">
                      <div className="flex-1 min-w-0">
                        <p className="text-xs text-slate-500 mb-1">ACID Number</p>
                        <p className="font-mono text-sm font-medium text-nafeza-600 break-all">{declaration.acidNumber}</p>
                      </div>
                      <span className={`${getStatusColor(declaration.status)} text-xs font-medium px-2.5 py-0.5 rounded flex-shrink-0`}>
                        {declaration.status}
                      </span>
                    </div>
                    <div>
                      <p className="text-xs text-slate-500 mb-1">Exporter</p>
                      <p className="text-sm text-slate-800">{declaration.exporterName}</p>
                    </div>
                    <div>
                      <p className="text-xs text-slate-500 mb-1">Date</p>
                      <p className="text-sm text-slate-800">{formatDate(declaration.requestDate)}</p>
                    </div>
                    <Button 
                      variant="outline" 
                      size="sm"
